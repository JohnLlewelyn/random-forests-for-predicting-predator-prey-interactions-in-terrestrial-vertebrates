#Random forest - both data types (phylo eigenvectors and ecomorph traits), many variables
...
df_opt_thrw <- replicate(100,rep_all_thrw(Int, Non, Non,ins=0.25,out=1,abs=5,thresh=0.46,mtr=2,max.depth=1000, trees=800)) #mean TSS~0.758 (if use 10 eig, 0.731; 20 = 0.743; 6=0.723)
saveRDS(df_opt_thrw, "###/GloBIonly_bothDataTypes_manyVar.rds")

#########################################################################################################################
SDall <- replicate(100,rf_thresh_all(Int,Non,ins=0.25,out=1,abs=5,SD_foc,thresh=0.46,mtr=2,max.depth=1000,num.trees=800)) #
mean(unlist(SDall[1,])) #TSS 
mean(unlist(SDall[2,])) #AUC 
saveRDS(SDall, "###/SDaspplied_bothDataTypes_manyVar.rds")
