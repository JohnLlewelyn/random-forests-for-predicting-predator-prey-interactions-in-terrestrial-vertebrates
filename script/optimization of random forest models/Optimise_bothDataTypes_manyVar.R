#Optimise random forest - model with many variables (both phylogenetic eigenvectors and ecomorph traits)
...
for (i in 1:20) {
  dt <- mean(replicate(40,rep_all_thrw(Int,Non,Non,ins=0.25,out=1,abs=i/2,thresh=0.42,mtr=14,max.depth=1000,num.trees = 800)))
  output[i,] <- c(round(dt,3),i/2)
  print(dt)
}

output=data.frame(output)
pdf("###/OpPEMs+Eco_abs_manyVar.pdf", height = 8, width = 16)
...
output=data.frame(output)
pdf("###/OpPEMs+Eco_ins-outs_manyVar.pdf", height = 8, width = 16)
plot(output$X2,output$X1) #peaks at
dev.off()
...
output=data.frame(output)
pdf("###/OpPEMs+Eco_mtry_manyVar.pdf", height = 8, width = 16)
...
output=data.frame(output)
pdf("###/OpPEMs_thresh21.pdf", height = 8, width = 16)
plot(output$X2,output$X1) 
dev.off()
