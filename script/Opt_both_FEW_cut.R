#optimise using both types of data (ecomorph and phylo eig), few variables, and the modified global dataset (predators with < 5 interaction records removed)
...
#cut to 5 most important eigenvectors and 5 ecomorph traits variables (from predator, and from prey)
impV <- readRDS("###/data/keepVar_both_cut.RDS")
Int <- Int[,names(Int)%in%c(names(impV),"outside","sourceTaxonName","targetTaxonName")]
Non <- Non[,names(Non)%in%c(names(impV),"outside","sourceTaxonName","targetTaxonName")]
...
#save results
saveRDS(glb,"###/results/Both_FEW_GLB_cut.RDS")

#save hyper parameters
hps <- data.frame(Hpar=c("abs","ins","mtry","thresh","ntrees","depth"),val=c(absv,insv,mtryv,threshv,ntreesv,mdepthv))
saveRDS(hps,"###/results/Hpars_Both_FEW_cut.RDS")

#If just running the SD part without above optimization script, get optimised parameters
hpars <- readRDS("~/Dropbox/Global Ecology Lab/research/Naracoorte network/ms/Ecography/1st revision/script/home/results/Hpars_Both_FEW_cut.RDS")
absv=hpars$val[hpars$Hpar=="abs"]
insv=hpars$val[hpars$Hpar=="ins"]
out=1
threshv=hpars$val[hpars$Hpar=="thresh"]
mtryv=hpars$val[hpars$Hpar=="mtry"]
mdepthv=hpars$val[hpars$Hpar=="depth"]
ntreesv = hpars$val[hpars$Hpar=="ntrees"]
...
saveRDS(SDp,"###/results/Both_FEW_SD_cut.RDS")
