...
preyChange_TDTC <- function(x,y,ins,out,abs,dt_test, thresh,mtr,percPrey,...){
  obs <-  x
...
output <- list()
for(i in 0:100){
  dt <- replicate(10,preyChange_TDTC(Int,Non,ins=1,out=1,abs=4,SD_foc,thresh=0.43,mtr=11,num.trees=800,max.depth=1000, percPrey = i))
  dt <- rbind(dt,i)
  output[[length(output) + 1]] <- dt   
}

saveRDS(output,"###/TDTC_preyChangInt2Non_results.rds")
