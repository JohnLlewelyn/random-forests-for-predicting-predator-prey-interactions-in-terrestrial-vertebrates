#Training dataset taxonomic coverage (TDTC) and performance - change focal prey interact to noninteract 
...
#the function for removing 
preyChange_TDTCmod <- function(x,y,ins,out,abs,dt_test, thresh,mtr,mdepth,ntrees,percPrey,...){
  obs <-  x
  ...
  SDprey <- data[data$targetTaxonName%in%prey&data$interact==TRUE,]
  Oprey <- data[!(data$targetTaxonName%in%prey&data$interact==TRUE),]
...
output <- list()
for(i in 0:100){
  dt <- replicate(10,preyChange_TDTCmod(Int,Non,ins=2.5,out=1,abs=4.75,SD_foc,thresh=0.31,mtr=42,ntrees=400,mdepth=0, percPrey = i))
  dt <- rbind(dt,i)
  output[[length(output) + 1]] <- dt  
  print(i)
}

saveRDS(output,"###/results/TDTC_preyChangInt2Non_results.rds")

#get the required data from the list together to plot
op <- data.frame(t(do.call("cbind", output)))
op <- data.frame(apply(op,2,unlist))
plot(op$i,op$Tsk, ylab = "TSS", pch = 19, cex = 0.5, col=rgb(red=0.1, green=0.2, blue=0.2, alpha=0.3))
