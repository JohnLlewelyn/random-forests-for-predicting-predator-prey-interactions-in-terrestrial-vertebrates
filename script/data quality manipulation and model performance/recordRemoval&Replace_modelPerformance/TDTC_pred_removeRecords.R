#Training dataset taxonomic coverage (TDTC) and performance - remove records for focal predators
...
prds_TDTCmod <- function(x,y,ins,out,abs,dt_test, thresh,mtr,mdepth,ntrees,percPred,...){
  obs <-  x
  ...
  SDprd <- data[data$sourceTaxonName%in%prds,]
  Oprd <- data[!(data$sourceTaxonName%in%prds),]
...
output <- list()
for(i in 0:100){
  dt <- replicate(10,prds_TDTCmod(Int,Non,ins=2.5,out=1,abs=4.75,SD_foc,thresh=0.31,mtr=42,ntrees=400,mdepth=0, percPred = i))
  dt <- rbind(dt,i)
  output[[length(output) + 1]] <- dt 
  print(i)
}

saveRDS(output,"###/results/TDTC_preds_results.rds")
