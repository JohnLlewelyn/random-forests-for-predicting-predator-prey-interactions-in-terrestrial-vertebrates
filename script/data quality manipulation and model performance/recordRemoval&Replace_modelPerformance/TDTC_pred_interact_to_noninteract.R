...
output <- list()
for(i in 0:100){
  dt <- replicate(10,prdChange_TDTC(Int,Non,ins=1,out=1,abs=4,SD_foc,thresh=0.43,mtr=11,num.trees=800,max.depth=1000, percPred = i))
  dt <- rbind(dt,i)
  output[[length(output) + 1]] <- dt   
}

saveRDS(output,"###/TDTC_preds_results_Int2Non.rds")
