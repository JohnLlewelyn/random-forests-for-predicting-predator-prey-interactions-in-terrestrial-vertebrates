#Training dataset taxonomic coverage (TDTC) and performance - remove interaction records of focal predator 
...
#get the data
Int <- readRDS("###/GloBIplus_Int.RDS")
Non <- readRDS("###/GloBIplus_noInt.RDS")
SD_foc <- readRDS("###/allperms_cut2.RDS")
...
prdChange_TDTC <- function(x,y,ins,out,abs,dt_test, thresh,mtr,percPred,...){
  obs <-  x
...
output <- list()
for(i in 0:100){
  dt <- replicate(10,prdChange_TDTC(Int,Non,ins=3,out=2,abs=2.5,SD_foc,thresh=0.49,mtr=13,num.trees=700,max.depth=1000, percPred = i))
  dt <- rbind(dt,i)
  output[[length(output) + 1]] <- dt   
}

saveRDS(output,"###/TDTC_preds_remove and performanc.rds")
...
#plot it
plot(op$i,op$Tsk,xlab="% of records of focal predator remaining in training dataset", ylab = "TSS", pch = 19, cex = 0.5, col=rgb(red=0.1, green=0.2, blue=0.2, alpha=0.3))
