#Training dataset size (TDS) and performance - only remove non-Simpson Desert species
...
#the function
rf_TDS <- function(x,y,ins,out,abs,dt_test, thresh,mtr,perc,...){
  obs <- x
  ...
  nonSD = data[!(data$sourceTaxonName%in%sp|data$targetTaxonName%in%sp),]
  SD = data[data$sourceTaxonName%in%sp|data$targetTaxonName%in%sp,]
  sel1 <- round(perc/100*dim(nonSD)[1])
  sel2 <- sample(1:dim(nonSD)[1],sel1)
  nonSD <- nonSD[sel2,]
  data <- rbind(SD, nonSD)
...
saveRDS(output,"###/TDS_results_nonSD.rds")

#get the required data from the list together
op <- data.frame(t(do.call("cbind", output)))
op <- data.frame(apply(op,2,unlist))

plot(op$i,op$Tsk,xlab="% of full training dataset", ylab = "TSS", pch = 19, cex = 0.5, col=rgb(red=0.1, green=0.2, blue=0.2, alpha=0.3))

#check at the extremes
dt0 <- replicate(40,rf_TDS(Int,Non,ins=3,out=2,abs=2.5,SD_foc,thresh=0.49,mtr=13,num.trees=700,max.depth=1000, perc = 0))
mean(unlist(dt0[1,])) #0.39025
dt100 <- replicate(40,rf_TDS(Int,Non,ins=3,out=2,abs=2.5,SD_foc,thresh=0.49,mtr=13,num.trees=700,max.depth=1000, perc = 100))
mean(unlist(dt100[1,])) #0.3793
