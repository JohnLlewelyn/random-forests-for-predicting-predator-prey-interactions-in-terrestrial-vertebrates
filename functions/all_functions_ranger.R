...
#build training and testing datasets, with pre-defined composition/connectance
train_test_conn <- function(x,y,z,ins,out,conn) { #ins and out = how many times more from inside versus outside suitable size range e.g., 10 and 1 or 1 and 10, abs = how many times more absent than present
  obs <- x
  obs$interact <- as.factor(TRUE)
  obs$outside = "present"
  y <- y[y$sourceTaxonName%in%unique(obs$sourceTaxonName),]
  unobs_in <- y[y$outside=="FALSE",]
  unobs_in <- unobs_in[sample(nrow(unobs_in), (dim(obs)[1]/(ins+out)*ins)*((1-conn)/conn), replace = FALSE),] #unobs for training
  unobs_out <- y[y$outside=="TRUE",]
  unobs_out <- unobs_out[sample(nrow(unobs_out), (dim(obs)[1]/(ins+out)*out)*((1-conn)/conn), replace = FALSE),]
  unobs <- rbind(unobs_in,unobs_out)
  #unobs$outside = NULL
  unobs$interact <- as.factor(FALSE)
  data <- rbind(obs,unobs[,names(obs)])
  ...
  test = rbind(test_obs,unobs2)
  test$outside = NULL
  train$outside <- ifelse(train$outside=="present",1,1/((1-conn)/conn))
  train_w <- train$outside
  train$outside = NULL
  return(list(train,test,train_w))
}

#unweighted but with specified composition/connectance
rep_all_nw_conn <- function(x,y,z,ins,out,conn,thresh,mtr, ...) {
  ch <- train_test_conn(x,y,z,ins,out,conn)
  ch[[1]] <- ch[[1]][,-tax_cols(ch[[1]])]
  ch[[2]] <- ch[[2]][,-tax_cols(ch[[2]])]
  rf <- ranger(formula = interact ~ ., # - targetTaxonGenusName - sourceTaxonGenusName, 
               data = ch[[1]],mtry=mtr,num.threads=20,probability = T) #
  pred <- predict(rf, data=ch[[2]][,-(which(names(ch[[2]])%in%c("interact","sourceTaxonName","targetTaxonName")))])
  #scores = predic$predictions[,1]
  #lbls <- dt_test$interact
  #lbls <- ifelse(lbls=="TRUE",1,0)
...
#unweighted and taking expected connectance into account when building training data
rf_thresh_nw_conn <- function(x,y,ins,out,conn,dt_test, thresh,mtr,...){
  obs <- x
  obs$interact <- as.factor(TRUE)
  obs$outside = "present"
  y <- y[y$sourceTaxonName%in%unique(obs$sourceTaxonName),]
  ...
  unobs$interact <- as.factor(FALSE)
  data <- rbind(obs,unobs)
  #data_w <- ifelse(data$interact=="TRUE",1,1/((1-conn)/conn))
  data$outside = NULL
  data = data[,-tax_cols(data)]  #ch[[2]] <- ch[[2]][,-tax_cols(ch[[2]])]
  #dt_test = dt_test[,!(tax_cols_sp(dt_test))]
  #try so there's an equal number observed vs unobserved in test data
...
#to replicate on global and specify tree depth
rep_all_mdepth <- function(x,y,z,ins,out,abs,thresh,mtr,mdepth, ...) {
  ch <- train_test_dfsw(x,y,z,ins,out,abs)
  ch[[1]] <- ch[[1]][,-tax_cols(ch[[1]])]
  ch[[2]] <- ch[[2]][,-tax_cols(ch[[2]])]
  rf <- ranger(formula = interact ~ ., # - targetTaxonGenusName - sourceTaxonGenusName, 
               data = ch[[1]],num.tree=800,mtry=mtr,num.threads=20,probability = T,case.weights = ch[[3]],max.depth = mdepth) #
  pred <- predict(rf, data=ch[[2]][,-(which(names(ch[[2]])%in%c("interact","sourceTaxonName","targetTaxonName")))])
  #scores = predic$predictions[,1]
  #lbls <- dt_test$interact
  #lbls <- ifelse(lbls=="TRUE",1,0)
...
###same as above but for Simp
rf_thresh_allmdepth <- function(x,y,ins,out,abs,dt_test, thresh,mtr,mdepth,...){
  obs <- x
  obs$interact <- as.factor(TRUE)
  obs$outside = "present"
  y <- y[y$sourceTaxonName%in%unique(obs$sourceTaxonName),]
